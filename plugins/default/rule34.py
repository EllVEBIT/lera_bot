if answ[1] == '34':
	...
	try:
		try:	
			...
			parse = untangle.parse('http://0s.oj2wyzjtgqxhq6dy.cmle.ru/index.php?page=dapi&s=post&q=index&limit=100&tags='+str(r34text))
			randnum = random.randint(0,len(parse.posts.post))
			#print(parse.posts.post[randnum])
			mess = 'Вот результаты по вашему запросу 😏<br>('+str(randnum)+'/'+str(len(parse.posts.post))+')<br>----------<br>Остальные теги: '+parse.posts.post[randnum]['tags']
